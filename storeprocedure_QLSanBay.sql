...
create proc sp_layVEMAYBAY_theoMASOVE(@MASOVE nvarchar(10))
as
select *,FORMAT(lb.NGAYKHOIHANH,'dd/MM/yyyy') as date
from VEMAYBAY vmb,HANHKHACH hk, LICHBAY lb, HANGHANGKHONG hhk, LOAIVE lv, CHUYENBAY cb, MAYBAY mb
where vmb.MASOVE=@MASOVE AND HK.MAHK = vmb.MAHK AND lb.MACHUYENBAY=vmb.MACHUYENBAY AND hhk.MAHANGHK = cb.MAHANGHK AND vmb.MALOAIVE=lv.MALOAIVE AND vmb.MACHUYENBAY=cb.MACHUYENBAY AND vmb.GIOKHOIHANH=lb.GIOKHOIHANH AND vmb.NGAYKHOIHANH=lb.NGAYKHOIHANH AND lb.SOHIEU= mb.SOHIEU

create proc sp_layDSVEMAYBAY_THEOMASOVE(@MASOVE nvarchar(10))
as
select *
from VEMAYBAY
where MASOVE=@MASOVE

create proc sp_layDSVEMAYBAY_THEOTENHHK(@TENHANGHK nvarchar(25))
as
select vmb.*
from VEMAYBAY as vmb, CHUYENBAY cb,HANGHANGKHONG hhk
where vmb.MACHUYENBAY=cb.MACHUYENBAY AND cb.MAHANGHK=hhk.MAHANGHK AND hhk.TENHANGHK=@TENHANGHK

create proc sp_layDSVEMAYBAY_THEOTENLOAIVE(@TENLOAIVE nvarchar(20))
as
select vmb.*
from VEMAYBAY as vmb, LOAIVE lv
where vmb.MALOAIVE=lv.MALOAIVE AND lv.TENLOAIVE=@TENLOAIVE
